...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("acsswitch://") {
        return false;
    }
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only the first acsswitch:// URL
                        }
                    }
                }
            });
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 acsswitch://...）
